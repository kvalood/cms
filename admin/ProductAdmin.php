...
class ProductAdmin extends Simpla
{
	...
	{	
		...
		if($this->request->method('post') && !empty($_POST))
		{
			...
			$product->ymarket = $this->request->post('ymarket', 'boolean');
			$product->google = $this->request->post('google', 'boolean');
				...
				if(empty($product->id))
				{
  	    		...
  	    		else
  	    		{
   	    		...
   	    		if($product->id)
   	    		{
					...
					$variant_images = $this->request->files('variant_images');
					
					if(!empty($variant_images))
					{
						//Сбрасываем ключи
						$variant_images['name'] = array_values($variant_images['name']);
						$variant_images['tmp_name'] = array_values($variant_images['tmp_name']);
					}
					
					//Проходимся по вариантам товара
					foreach($variants_ids as $id_variant => $value)
					{
						/***ДОбавляем картинки, если они найдены в этом варианте товара.
						*	Измеряем размер массива
						*	Проверяем, нормальное ли имя суб элемента, если да, то добавляем картинку.
						*/
						
						for($id_image=0;$id_image<=count($variant_images['name']);$id_image++)
						{
							if(!empty($variant_images['tmp_name'][$id_variant][$id_image]))
							{	
								if($image_name = $this->image->upload_image($variant_images['tmp_name'][$id_variant][$id_image], $variant_images['name'][$id_variant][$id_image]))
								{
									$this->products->add_image($product->id, $image_name, '', $value);
								}
								else
								{
