...
class Delivery extends Simpla
{
	...
	{
		$query = $this->db->placehold("SELECT id, name, description, period, free_from, price, enabled, position, separate_payment FROM __delivery WHERE id=? LIMIT 1", intval($id));
	...
	{	
		...
		$query = "SELECT id, name, description, period,  free_from, price, enabled, position, separate_payment
