...
class Design extends Simpla
{
	...
	public function none_modifier($filename)
	{
		return $this->config->root_url.'/'.$this->config->original_images_dir.$filename;
	}
	...
	*/
    ...
    public function js_code_block($params, $content, $smarty)
    {
        $this->smarty->append('footer_code', $content);
    }

    public function footer_code_function() {

        $footer_code = (array) $this->smarty->getTemplateVars('footer_code');
        $footer_code = array_unique($footer_code);
        if(isset($footer_code)) {
            foreach ($footer_code as $item) {
                echo $item ."\n";
            }
        }
    }
