...
class Image extends Simpla
{
	...
	public function upload_image($filename, $name, $method = 'product')
	{
		// Имя оригинального файла
		...
		{	
			...
			{
				case 'product': 
					$image_dir = $this->config->root_dir.$this->config->original_images_dir;
				break;

                // TODO: Зачем тут разбиение на типы??
				case 'article':
					$image_dir = $this->config->root_dir.$this->config->article_images_dir;
				break;
			}
			
			while(file_exists($image_dir.$new_name))
			{	
				$new_base = pathinfo($new_name, PATHINFO_FILENAME);
				if(preg_match('/_([0-9]+)$/', $new_base, $parts))
					$new_name = $base.'_'.($parts[1]+1).'.'.$ext;
			...
			if(move_uploaded_file($filename, $image_dir.$new_name))			
				return $new_name;
		}
	...
	private function image_constrain_imagick($src_file, $dst_file, $max_w, $max_h, $watermark=null, $watermark_offet_x=0, $watermark_offet_y=0, $watermark_opacity=1, $sharpen=0.2)
	{
        ...
        if(exif_imagetype($src_file) == IMAGETYPE_PNG)
        {
            $im = imagecreatefrompng($dst_file);
            imagepng($im, $dst_file, 9);
            imagedestroy($im);
        }
    ...
    {
        // Имя оригинального файла
        $name = $this->correct_filename($file['name']);
        $uploaded_file = $new_name = pathinfo($name, PATHINFO_BASENAME);
        $base = pathinfo($uploaded_file, PATHINFO_FILENAME);
        $ext = pathinfo($uploaded_file, PATHINFO_EXTENSION);
        ...
        if(in_array(strtolower($ext), $this->allowed_extentions))
        {
            $path = $this->config->root_dir . DS . $upload_path;
            // Создаем папку, если её нет.
            if(!is_dir($path)) {
                if (!mkdir($path, 0777, true)) {
                    die('Failed to create folder ' . $upload_path);
                }
            }

            while(file_exists($path . DS . $new_name))
            {
                $new_base = pathinfo($new_name, PATHINFO_FILENAME);
                if(preg_match('/_([0-9]+)$/', $new_base, $parts))
                    $new_name = $base.'_'.($parts[1]+1).'.'.$ext;
