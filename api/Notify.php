...
class Notify extends Simpla
{
	...
	public function email_order_user($order_id)
	{
        ...
        $this->email($order->email, $subject, $email_template, $this->settings->site_name.' <'.$this->settings->notify_from_email.'>');
	}
	...
	public function email_order_admin($order_id)
	{
        ...
        // Отправляем письмо
        $email_template = $this->design->fetch($this->config->root_dir.'design/'.$this->settings->theme.'/html/email/email_order_admin.tpl');
        $subject = $this->design->get_var('subject');
        $this->email($this->settings->order_email, $subject, $email_template, $this->settings->site_name.' <'.$this->settings->notify_from_email.'>');
	...
	{ 
        ...
        // Отправляем письмо
        $email_template = $this->design->fetch($this->config->root_dir.'design/'.$this->settings->theme.'/html/email/email_comment_admin.tpl');
        $subject = $this->design->get_var('subject');
        $this->email($this->settings->comment_email, $subject, $email_template, $this->settings->site_name.' <'.$this->settings->notify_from_email.'>');
	}

    public function email_comment_retly($comment_id)
    {
        ...
        {
            $comment->article = $this->article->get_article(intval($comment->object_id));
            $comment->article_category = $this->article->get_category(intval($comment->article->id));
        }

        $this->design->assign('comment', $comment);
        $email_template = $this->design->fetch($this->config->root_dir.'design/'.$this->settings->theme.'/html/email_comment.tpl');
        $subject = $this->design->get_var('subject');

        $this->email($comment->email, $subject, $email_template, $this->settings->site_name.' <'.$this->settings->notify_from_email.'>');
    }
