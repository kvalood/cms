...
class CartView extends View
{
  ...
  public function __construct()
  {
    ...
    if(isset($_POST['checkout']))
    {
		...
		$count_phone        = (int) strlen($order->phone);
    	...
    	if(isset($order->name) AND empty($order->name))
    	{
    		$this->design->assign('error', 'empty_name');
    	}
    	elseif(isset($order->email) AND !filter_var($order->email, FILTER_VALIDATE_EMAIL))
    	{
    		$this->design->assign('error', 'no_email');
    	}
    	elseif(isset($captcha_code) AND ($_SESSION['captcha_code'] != $captcha_code))
    	{
    		$this->design->assign('error', 'captcha');
    	}
		elseif(isset($order->phone) AND ($count_phone > 14 || $count_phone < 6))
    	{
    		$this->design->assign('error', 'error_phone');
    	}
    	else
    	{
