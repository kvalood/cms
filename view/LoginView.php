...
class LoginView extends View
{
	function fetch()
	{
		...
		// Вход
		elseif($this->request->method('post') && $this->request->post('login') && $this->settings->user_login)
		{
			$email			= $this->request->post('email');
			...
			if($user_id = $this->users->check_password($email, $password))
			{
				...
				if($user->enabled)
				{
					...
					$this->users->update_user($user_id, array('last_ip'=>$_SERVER['REMOTE_ADDR'], 'last_visit' => date('Y-m-d H:i:s')));
